  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage(json?.message ?? "Nao foi possivel iniciar sessao.");
        return;
      }

      const json = await response.json().catch(() => null);
      onLogin(json?.userName ?? null);
      onNavigate("home");
    } finally {
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center flex-1 z-10 w-full max-w-[1600px] mx-auto">
        <div className="w-full max-w-md bg-[#0a0a0a]/60 backdrop-blur-xl border border-white/10 p-10 md:p-12 rounded-[30px] shadow-2xl relative">
          ...
          <p className="text-white/50 text-sm font-mono tracking-wide mb-8">Introduz as tuas credenciais para entrar.</p>

          <form className="space-y-4" onSubmit={submit}>
            {message && (
              <div className="border border-yellow-400/25 bg-yellow-400/10 text-yellow-100 rounded-xl px-4 py-3 text-xs font-mono">
                {message}
              </div>
            )}
            ...
            <button
              ...
              className="w-full group/btn relative flex items-center justify-center gap-2 bg-yellow-400 text-black px-8 py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-yellow-300 transition-all duration-300 mt-8 shadow-[0_0_20px_rgba(250,204,21,0.15)] hover:shadow-[0_0_30px_rgba(250,204,21,0.3)] disabled:opacity-70"
            >
          ...
          <div className="mt-8 text-center border-t border-white/5 pt-8">
            <p className="text-xs text-white/50 font-mono tracking-wide">
              ...
              <button type="button" onClick={() => onNavigate("register")} className="text-yellow-400 hover:text-yellow-300 uppercase tracking-widest font-bold ml-1 transition-colors">
                Registar
              </button>
            </p>
          </div>
