...
public class EventosApiController : ControllerBase
{
    ...
    {
        var eventoExiste = await _context.Eventos.AnyAsync(e => e.IdEvento == id, ct);
        if (!eventoExiste)
            return NotFound(new { message = "Evento nao encontrado." });

        if (string.IsNullOrWhiteSpace(req.Nome))
            return BadRequest(new { message = "O nome e obrigatorio." });

        if (string.IsNullOrWhiteSpace(req.Local))
            return BadRequest(new { message = "O local e obrigatorio." });

        if (req.Capacidade <= 0)
            return BadRequest(new { message = "A capacidade tem de ser maior que 0." });

        var categoriaExiste = await _context.Categorias.AnyAsync(c => c.IdCategoria == req.IdCategoria, ct);
        if (!categoriaExiste)
